#!/bin/bash
#
#SBATCH --job-name=_demo__04_downsample
...
# for d in ${dems[@]}; do
	...
	for i in $(seq 0 $cvgCnt); do
		...
		CVG_OUTPUT_DIR=${OUTPUT_DIR}/_demo__sample_cvg_${cvgX[i]}
		...
		while read -a line; do
			...
			OUT_FILE=_demo__${line[0]}_cvg_${cvgX[i]}.bam
			start_logging "samtools view - ${OUT_FILE}"

			samtools view -s ${cvgP[i]} -@ 19 \
				-o ${CVG_OUTPUT_DIR}/${OUT_FILE} \
				${INPUT_DIR}/_demo_/_demo__${line[0]}_genome_sorted.bam
