#!/bin/bash
#
#SBATCH --job-name=large-1000_04_downsample
...
# for d in ${dems[@]}; do
	...
	for i in $(seq 0 $cvgCnt); do
		...
		CVG_OUTPUT_DIR=${OUTPUT_DIR}/large-1000_sample_cvg_${cvgX[i]}
		...
		while read -a line; do
			...
			OUT_FILE=large-1000_${line[0]}_cvg_${cvgX[i]}.bam
			start_logging "samtools view - ${OUT_FILE}"

			samtools view -s ${cvgP[i]} -@ 19 \
				-o ${CVG_OUTPUT_DIR}/${OUT_FILE} \
				${INPUT_DIR}/large-1000/large-1000_${line[0]}_genome_sorted.bam
