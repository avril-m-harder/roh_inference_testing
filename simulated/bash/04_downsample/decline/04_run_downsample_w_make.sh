#!/bin/bash
#
#SBATCH --job-name=decline_04_downsample
...
# for d in ${dems[@]}; do
	...
	for i in $(seq 0 $cvgCnt); do
		...
		CVG_OUTPUT_DIR=${OUTPUT_DIR}/decline_sample_cvg_${cvgX[i]}
		...
		while read -a line; do
			...
			OUT_FILE=decline_${line[0]}_cvg_${cvgX[i]}.bam
			start_logging "samtools view - ${OUT_FILE}"

			samtools view -s ${cvgP[i]} -@ 19 \
				-o ${CVG_OUTPUT_DIR}/${OUT_FILE} \
				${INPUT_DIR}/decline/decline_${line[0]}_genome_sorted.bam
