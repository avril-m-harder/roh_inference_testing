...
#SBATCH --job-name=05c_combine_and_genotype_gvcfs
...
for d in ${dems[@]}; do
	...
	for c in ${cvgX[@]}; do 
		...
		MAP_FILE=${OUTPUT_DIR}/${d}_sample_cvg_${c}_map.list
		ls ${d}_sample_cvg_${c}/*.g.vcf > ${MAP_FILE}
	

		OUT_FILE=${d}_sample_cvg_${c}_combined_gvcfs.vcf
		...
		OUT_FILE=${d}_sample_cvg_${c}_genotyped_gvcfs.vcf
