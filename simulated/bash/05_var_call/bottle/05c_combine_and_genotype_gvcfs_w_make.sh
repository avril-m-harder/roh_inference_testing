...
#SBATCH --job-name=bottle_05c_combine_and_genotype_gvcfs
...
for c in ${cvgX[@]}; do 
	...
	MAP_FILE=${OUTPUT_DIR}/bottle_sample_cvg_${c}_map.list
	ls bottle_sample_cvg_${c}/*.g.vcf > ${MAP_FILE}
	

	OUT_FILE=bottle_sample_cvg_${c}_combined_gvcfs.vcf
	...
	OUT_FILE=bottle_sample_cvg_${c}_genotyped_gvcfs.vcf
