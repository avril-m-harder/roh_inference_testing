...
for d in ${dems[@]}; do
	for c in ${cvgX[@]}; do 
		...
		OUT_FILE=${d}_sample_cvg_${c}_flagged_gvcfs.vcf
		FLAGGED_FILE=${OUT_FILE}
		GENOTYPED_FILE=${d}_sample_cvg_${c}_genotyped_gvcfs.vcf
		...
		OUT_FILE=${d}_sample_cvg_${c}_filtered_SNPs.vcf
