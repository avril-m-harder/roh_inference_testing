...
## filename format: bottle_sample_cvg_${c}_PL_VT_ONLY.txt
fns <- list.files()
fns <- fns[grep('bottle', fns)]
...
for(d in c('GT','PL')){
  OUT <- NULL
  for(c in covgs){
    dat <- read.table(paste0('bottle_sample_cvg_',c,'_',d,'_VT_ONLY.txt'))
    dat <- dat[,-c(1,6,8)]
  ...
  write.table(OUT1, paste0('bottle_',d,'_viterbi_estimates.txt'), sep = '\t', col.names = FALSE, row.names = FALSE, quote = FALSE)
}