...
#SBATCH --job-name=small_06b_run_PLINK
#SBATCH -N 1
#SBATCH -n 20
#SBATCH -t 12:00:00
#SBATCH --mem=32000
...
for c in ${cvgX[@]}; do
	...
	OUT_FILE=small_sample_cvg_${c}_plink
	...
	declare -a phwh=(1)         # Values for -homozyg-window-het
						...
						for p6 in ${phwt[@]}; do         # -homozyg-window threshold
							for p7 in ${phzs[@]}; do     # -homozyg-snp
								for p8 in ${phzk[@]}; do # -homozyg-kb
									...
									IN_FILE=small_sample_cvg_${c}_plink
									OUT_FILE=small_sample_cvg_${c}_plink_roh${PARAM_SUFFIX}
