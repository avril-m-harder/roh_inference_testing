...
###### Simulated Data ######
setwd('/Users/Avril/Desktop/roh_param_project/iterative_sa_steps/simulated/')
...
# rds <- c('round1','round2','round2a','round2b','round2c','round2d','round2e','round2f','round2g')
rds <- c('round1','round2','round2a','round2f','round2g')
...
for(rd in rds){
  ...
  for(c in sort(unique(df.iter$covg))){
    ...
    if(c == 5){
      write.table(OUT, paste0('true_v_call_fROH_data/',rd,'_true_vs_called_fROH_indiv_data.txt'),
    ...
    if(length(unique(origMelt1$Variables)) > 2){
      pdf(paste0('figures/simulated_',rd,'_',c,'X_paramvsSRC.pdf'), width = 6, height = 5)
    } else{
      pdf(paste0('figures/simulated_',rd,'_',c,'X_paramvsSRC.pdf'), width = 3, height = 5)
        xmin <- 0.65
        xmax <- max + 0.35
    }
    x <- 1
    ...
    pdf(paste0('figures/simulated_',rd,'_',c,'X_truefROH_vs_SRC.pdf'), width = 7, height = 5)
    ...
    pdf(paste0('figures/simulated_',rd,'_',c,'X_callfROH_vs_SRC.pdf'), width = 6.25, height = 5)
    par(mai = c(1.02,0.82,0.82,1.42))
    ...
    pdf(paste0('figures/simulated_',rd,'_',c,'X_true_v_called_fROH.pdf'), width = 7.05, height = 5)
    par(mai = c(1.02,0.87,0.82,1.42))
